...
#   - Ex: docker.io/hexpm/elixir:1.17.2-erlang-27.0.1-debian-trixie-20260518-slim
#
...
# (curl + Node.js are needed to `npm ci` the JS deps, e.g. pixi.js, that esbuild bundles)
RUN apt-get update \
...
# install JS deps (pixi.js, etc.) so esbuild can resolve bare imports
